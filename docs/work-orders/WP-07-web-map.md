...
- 상태: **완료 (2026-07-22)** — 구현 `595ea25`, §2 전 항목 통과: (a) 테스트 13건 (b) lint/build (c) 프록시 bbox 200 (d) 배지·mono 폰트·마커 12개·이동 후 배지 변경(50건→0건→8건, 로딩/0건 상태 포함) (e) 마커 클릭→상세 이동 (f) 적대적 리뷰 후 반영 — **재시도 영구 고착(next/script 캐시) 수정이 핵심**: 재시도마다 src에 캐시버스터를 붙여 실제 재로드(차단→복구 실증). 그 외 실패 시 stale 마커 제거, 동일 좌표 마커 오프셋, cleanup의 debounce 정리, bbox 응답 배열 가드, 500건 도달 시 "N건+" 표시. §0(NCP 콘솔 Web URL 등록)은 이미 완료돼 있었음(타일 인증 통과 실확인). 참고: 리뷰의 "50건 cap" 지적은 오탐 — DB 전체가 50건(수집기 5페이지 적재분). | 선행: 없음

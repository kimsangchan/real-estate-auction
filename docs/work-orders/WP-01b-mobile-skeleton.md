...
- 상태: **완료 (2026-07-22 문서 보완)** — 구현은 `4550c83`(RN 0.86 스캐폴드)에서 완료됐으나 상태 갱신이 누락됐었음. 이후 지도 홈·목록·상세·체크리스트 화면(F-01, F-04)이 이 골격 위에 구현·구동됨(에뮬레이터 AVD `AuctionTest` 실행 흔적: WP-06 §3). lint/test/tsc 통과 재확인(2026-07-22, 11 tests). iOS는 Windows 작업 환경으로 macOS 검증 필요 상태(완료 기준의 조건부 통과 조항 적용). | 선행: WP-01(완료)

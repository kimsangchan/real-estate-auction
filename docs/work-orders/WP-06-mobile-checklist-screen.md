...
- 상태: **완료 (2026-07-22)** — 구현 `04e9ba2`, §2 전 항목 통과: jest 15건·tsc·lint ✅, Gradle assembleDebug ✅ (주의: JAVA_HOME이 Java 6을 가리키면 wrapper가 죽음 — JDK 21 경로 지정 필요), 에뮬레이터 E2E ✅ (상세→권리분석→위험→체크리스트→2/4 체크→force-stop→재실행→2/4 복원, 스크린샷 검증), 적대적 리뷰 1회 후 지적 5건 반영(읽기 실패 시 쓰기 게이트 차단, 진입 링크 접근성, Props 타입 강화, 값 타입 검증, 병합·실패 경로 테스트 추가). | 선행: 없음

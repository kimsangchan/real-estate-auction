...
- 상태: **완료 (2026-07-22)** — 서버(dd650de)+웹(3d4d1f7)+리뷰반영(20fcdf6). §2 전 항목 통과: (a) 마이그레이션 002 멱등 (b) API 253·웹 18 테스트 (c) lint/build (d) **실계정 카카오 로그인 E2E 실증** — 로그인→app_user 1건 생성(id_token 검증 통과)·refresh_token 발급, 물건 관심 등록→favorite 1건(auction_item 4중 조인 매칭 확인)→목록 표시→해제→favorite 0건 전 왕복을 화면+DB 대조로 검증 (e) 비로그인 회귀 4화면 200 (f) 적대적 리뷰 Critical 0건, 반영 4건. 카카오 Redirect URI는 사용자가 콘솔 등록 완료. 네이버는 콜백 배선만 동일(미검증). | 선행: 없음

...
1. **제공자 범위**: 서버는 provider 플러그인 구조로 3종(카카오·네이버·애플)을 수용하되, **이번 구현·검증은
   카카오+네이버**. 애플 로그인은 iOS 출시 게이트(로드맵 3-3, Apple 지침 4.8)와 함께 별도 WP —
   Android/웹 MVP에서 애플 로그인은 심사 요건도 사용 수요도 없다. 결정(3종) 자체는 유지, 시점만 단계화.
...
- 애플 로그인(iOS 출시 WP), 모바일 로그인·관심 탭(WP-08b), 푸시·변동 감지(WP-09),

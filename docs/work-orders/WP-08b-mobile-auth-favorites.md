...
- 상태: **완료 (2026-07-27)** — §2-a~h 전부 통과. 카카오 실계정 모바일 로그인 E2E(딥링크 복귀·관심
  등록/해제·강제종료 후 세션 유지)까지 실증. 적대적 리뷰 2회 반영(Critical 1건 포함).
  스토어 제출 전 처리해야 할 미결 설계 사항은 §4 참고.
...
- [x] d. 모바일 E2E **카카오 실계정으로 전 구간 통과 (2026-07-27, 에뮬레이터 `-gpu host`)**
      - 관심 탭 비로그인 안내 → 로그인 화면 → 카카오 버튼 → 시스템 브라우저 `accounts.kakao.com`
        → 실계정 로그인 → **딥링크로 앱 복귀 후 로그인 상태**(DB: 08:31:27 리프레시 토큰 신규 발급)
      - 물건 상세 관심 등록 → DB `favorite` 1건(B000210/2025타경939/3) → 관심 탭에 카드 표시
        → 관심 해제 → DB 0건
      - **앱 강제종료 후 재실행 시 세션 유지 확인** — 브라우저 없이 "카카오 사용자님" 렌더,
        DB에서 토큰이 08:31:27(폐기) → 08:35:39(신규)로 회전. Keystore 읽기 → `/auth/refresh`
        → 회전 → 재저장이 실동작.
      - 딥링크 intent-filter 별도 확인: 무효 코드는 크래시·로그아웃 없이 무시(리뷰 finding #4 기기 확인).
      - 네이버는 배선만(웹 WP-08과 동일 기준).

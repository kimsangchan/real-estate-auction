...
- 상태: **진행 중 (2026-07-27)** — 구현·테스트·적대적 리뷰 반영 완료. §2-a·b·c·e·f·g·h 통과.
  **남은 것은 §2-d의 카카오 실계정 로그인 한 단계뿐**(사용자 액션). 미결 설계 사항은 §4.
...
- [~] d. 모바일 E2E (AVD, §3-1 adb reverse 선행) — **가는 길은 확인, 돌아오는 길은 사용자 액션 대기**
      - 확인됨(에뮬레이터 `-gpu host`, adb reverse 3000/4000/8081):
        지도·목록·관심 탭 렌더 → 관심 탭 비로그인 안내 → 로그인 화면 → 카카오 버튼 →
        시스템 브라우저가 **실제 `accounts.kakao.com` 로그인 페이지까지 도달**
        (= `Linking.openURL` → adb reverse → Next 프록시 → API의 `client=mobile`·codeChallenge
        검증 → state 서명 → 카카오 리다이렉트가 전부 실동작). 딥링크 intent-filter도 확인 —
        `auction://auth/callback?code=…`가 앱으로 라우팅되고, 무효 코드는 크래시·로그아웃 없이
        무시된다(적대적 리뷰 finding #4 수정의 기기 확인).
      - **남은 것: 카카오 실계정 자격증명 입력.** 그 뒤 딥링크 복귀 → 관심 등록/해제 →
        앱 강제종료 후 세션 유지(Keystore)까지가 미검증. 네이버는 배선만(웹 WP-08과 동일 기준).
...
1. ~~리프레시 재사용 감지가 서버에서 무효 상태~~ **해결 (2026-07-27, 사용자 확정)** —
   롤백 버그를 고치고(트랜잭션 안에서 throw하지 않고 결과를 값으로 반환) **10초 유예 창**을 도입했다.
   회전 직후 재제출은 `RefreshRaceError`로 거절만 하고 계열을 폐기하지 않으며, 컨트롤러는 이때
   쿠키를 건드리지 않는다(이긴 요청이 심은 세션 보존). 실서버 확인: 동시 6건 → `[200,401×5]`이고
   경쟁 후에도 세션 생존, 유예 창 밖 재사용은 계열이 실제로 폐기됨(폐기가 커밋됨).

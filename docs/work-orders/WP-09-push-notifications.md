...
- 상태: **완료 (2026-07-29)** — §2-a~h 전부 통과. 로드맵 2-6의 "알림 수신 E2E 1건"을 실증해
  2-6(소셜 로그인 + 관심 물건 + 푸시)을 종결한다. 적대적 리뷰 2회 반영 후 기기 수신까지 재확인.
  알려진 한계는 §5, 리뷰 반영 내역과 남은 지적은 §6 참고.
...
- [x] d. **알림 수신 E2E 1건 (로드맵 2-6 완료 기준) — 통과, 적대적 리뷰 반영 후 재확인 (2026-07-29)**:
      에뮬레이터에서 세션 복구 → 권한 이미 허용이라 창 없이 토큰 재등록(DB `last_seen_at` 갱신) →
      변동 주입 → `notify` 실행 → **기기 알림 트레이에 수신**(`vis=PRIVATE` 적용 확인) → 탭 →
      해당 물건 상세로 이동. 재실행 시 `deduped:1, sent:0`으로 중복 발송 없음.
      수신 문구: "6회 유찰돼 최저가가 2억 9,523만원이 됐어요. 매각기일이 2026년 8월 12일 오전

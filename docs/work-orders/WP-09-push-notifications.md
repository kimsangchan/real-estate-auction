...
- [ ] a. API 테스트: 변동 감지가 "마지막 처리 시각 이후 신규 schedule 행"만 잡는지, 같은 변동을
      두 번 실행해도 발송 0건인지(멱등), 관심 등록자가 없는 물건은 발송 0건인지, 조용한 시간 지연,
      D-3·D-1 경계값, 죽은 토큰 삭제 — `pnpm --filter @auction/api test` 전체 통과
- [ ] b. 모바일 테스트: 토큰 등록/삭제 호출, 권한 거부 시에도 화면이 정상 동작, 알림 페이로드 파싱 →
      상세 라우팅 파라미터 — `pnpm --filter @auction/mobile test` 통과
- [ ] c. `pnpm -r lint && pnpm -r test && pnpm -r build` + Gradle `assembleDebug` 통과
- [ ] d. **알림 수신 E2E 1건 (로드맵 2-6 완료 기준)**: 에뮬레이터에서 로그인 → 물건 관심 등록 →
      DB에 변동(신규 schedule 행) 주입 → 알림 잡 실행 → **기기에서 알림 수신** → 탭 → 해당 물건
      상세로 이동까지 스크린샷으로 확인
- [ ] e. 비대상 회귀: 관심 등록하지 않은 물건의 변동에는 발송 0건, 로그아웃 상태에서 토큰이 지워져
      더 이상 수신되지 않음
- [ ] f. 문구 검사: 발송 카피 전수에 판단·권유 표현 0건 (D-011)
- [ ] g. 적대적 리뷰 1회(새 컨텍스트, diff + 이 완료 기준만) 후 지적 반영

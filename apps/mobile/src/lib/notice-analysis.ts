// 매각물건명세서 기반 권리분석 응답 타입 (apps/api NoticeAnalysisDto와 1:1) + 인수액 합산 규칙.
//
// 웹 원본: apps/web/app/items/notice-analysis.ts — 같은 정의를 앱에도 둔다(별도 앱이라 import 불가,
// notice-labels.ts와 같은 선례). 한쪽만 고치면 웹/앱의 인수액 계산이 갈라지므로 **함께** 고쳐야 한다.
//
...
export function assumedTotal(tenants: readonly AnalyzedTenant[]): {
  ...
  for (const tenant of tenants) {
    if (tenant.assumption === 'ASSUMED_FULL' && tenant.assumedAmount !== null) {
      ...
      tenant.assumption === 'ASSUMED_AMOUNT_UNKNOWN' ||
      tenant.assumption === 'UNKNOWN'
    ) {
      isLowerBound = true;
 ...
 */
export function assumedHeadline(total: {
  amount: number;
  isLowerBound: boolean;
}): AssumedHeadline {
  if (total.amount > 0) {
    return {
      kind: 'AMOUNT',
      amount: total.amount,
      isLowerBound: total.isLowerBound,
    };
  }

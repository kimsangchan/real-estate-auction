...
//
// 웹 원본: apps/web/app/items/affordability.ts — 같은 정의를 앱에도 둔다(별도 앱이라 import 불가).
// 금액 포맷만 RN용 format.ts를 쓴다(Hermes에 Intl이 없어 웹과 구현이 다르다). 함께 고쳐야 한다.
//
...
/**
 * 요약에 쓸 대표 시나리오 — 유사 물건 중간 가격대가 있으면 그것, 없으면 최저가.
 * 최저가 가정은 경쟁이 붙는 물건에서 비현실적이라 중위를 우선한다.
 */
export function summaryScenario(
  affordability: Affordability,
): AffordabilityScenario | null {
  return (
